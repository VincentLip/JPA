...
public class ProduitService extends BaseService implements Repository<Produit> {
    ...
    public List filterByNote() {
        session = sessionFactory.openSession();
        NativeQuery query = session.createSQLQuery("select marque,dateAchat,reference,prix,stock from Produit as p inner join Comment as c on p.id = c.produit_id where c.note>4");
//        Query<Produit> produitQuery = session.createQuery("select Produit from Produit inner join Comment on Produit.id = Comment.id where Comment.note >4");
//        List<Produit> produitList = produitQuery.list();
